    ...
    usuariosStorage.forEach((usuario, key) => {
        if (!usuarioSesion || ( !usuariosStorage[usuarioSesion.id].siguiendo.includes(usuario.usuario) && usuario.usuario != usuarioSesion.usuario)) {
            list += `<div class="row my-4">
                ...
                <div class="col-5">
                    <button type="button" class="border-0 btn btn-brand-white text-brand text-nowrap rounded-0 px-5 px-md-3 my-2" onclick="globalFollowUser('${usuario.usuario}')"><b>Follow</b></button>
                </div>
            </div>`;
        }
    });
